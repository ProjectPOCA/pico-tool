...
LOG_PATH = "/state/vusion_4in2_poca_main_log.txt"
RUN_PATH = "/state/vusion_4in2_poca_main_run_id.txt"
...
PLANE_BLACK_INVERT = True
PLANE_RED_INVERT = True
...
def init_se2417(epd):
    ...
    epd.cmd(0x04)
    epd.wait_busy_high("se2417_pon")
    epd.cmd(0x00, bytes((0x0F, 0x0D)))
    epd.cmd(0x50, bytes((0x77,)))
    log("INIT se2417 done busy={}".format(epd.busy.value()))


def refresh(epd):
    epd.cmd(0x12)
    time.sleep_ms(REFRESH_CMD_GUARD_MS)
    epd.wait_busy_high("refresh_12")
...
def draw_settings(buf, profile):
    ...
    text(buf, 24, 152, "SE2417 DRIVER ACTIVE")
...
def _payload_from_mask(mask, invert):
    if invert:
        return bytes((b ^ 0xFF) for b in mask)
    return mask


def write_frame(epd, black_mask, red_mask):
    p_black = _payload_from_mask(black_mask, PLANE_BLACK_INVERT)
    p_red = _payload_from_mask(red_mask, PLANE_RED_INVERT)
    epd.cmd(0x10, p_black)
    epd.cmd(0x13, p_red)
    refresh(epd)
...
def boot_preflight(epd):
    if not PREFLIGHT_BOOT:
        return
    white = bytes((0xFF,)) * BUF_LEN
    black = bytes((0x00,)) * BUF_LEN
...
def main():
    try:
        run = read_run_id() + 1
        write_run_id(run)
        log("=== 4IN2 POCA MAIN RUN {} START ===".format(run))
        log("FW {}".format(os.uname().version))
        log("GEOM {}x{}".format(W, H))
